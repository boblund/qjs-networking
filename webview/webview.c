...
static void webview_gc_mark(JSRuntime *rt, JSValueConst val, JS_MarkFunc *mark_func) {
    webview_data_t *data = JS_GetOpaque(val, js_webview_class_id);
    if (!data) return;
    bind_ctx_t *bc = data->binds;
    while (bc) {
        JS_MarkValue(rt, bc->func, mark_func);
        bc = bc->next;
    }
}
...
static bool js_is_promise_like(JSContext *ctx, JSValueConst val) {
    if (!JS_IsObject(val)) return false;
    JSValue then_fn = JS_GetPropertyStr(ctx, val, "then");
    bool is_promise = JS_IsFunction(ctx, then_fn);
...
static void bind_callback(const char *seq, const char *req, void *arg) {
    ...
    if (JS_IsException(ret)) {
        ...
        result_val = JS_GetException(ctx);
    } else if (js_is_promise_like(ctx, ret)) {
				JSContext *ctx1;
				...
				JS_FreeValue(ctx, ret);   /* the Promise object itself — no longer needed once its result is extracted */
		}
